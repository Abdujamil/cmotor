<template>
  <div class="nav">
    ...
    <div class="nav__items">
      ...
      <div v-if="isManagersPage" class="nav__item nav__item-btn" @click="toggleSubmenu('managerNps')">
        Доступ к менеджерам
        <span
          class="nav__arrow"
          :class="{ 'nav__arrow--open': showNPSManagerSubmenu }"
        >
          <!-- SVG стрелки -->
          <svg
            xmlns="http://www.w3.org/2000/svg"
      ...
      <div v-if="showNPSManagerSubmenu" class="nav__submenu">
        <div
          class="nav__item"
          :class="{ 'nav__item--active': activeSubmenu === 'npsManager' }"
          @click="setActiveSubmenu('npsManager')"
        >
          <RouterLink class="nav__link" to="/manager"
            >Менеджеры</RouterLink
          >
        </div>
      </div>
    </div>
...
const toggleSubmenu = (submenu) => {
  ...
  }else if (submenu === "managerNps") {
    showNPSManagerSubmenu.value = !showNPSManagerSubmenu.value;
    // showCallsSubmenu.value = false;
    // showStatsSubmenu.value = false;
    // activeMenu.value = "nps";
  }
